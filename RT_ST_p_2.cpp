...
void RT_ST_p_2()
{
  ...
  double n_density2, n_charged2, density_factor, Nch, Rt, St, pT_sum2, meanPtp2, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  pttree2 ->SetBranchAddress("meanPtp2", &meanPtp2);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtp2, 1);
      hprof2->Fill(Rt, meanPtp2, 1);
  ...
  myfunc1->SetMaximum(2.0);
  myfunc1->SetMinimum(1.4);
  myfunc1->SetTitle("");
