...
void RT_ST_p_1()
{
  ...
  double n_density1, n_charged1, density_factor, Nch, Rt, St, pT_sum1, meanPtp1, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  pttree1 ->SetBranchAddress("meanPtp1", &meanPtp1);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtp1, 1);
      hprof2->Fill(Rt, meanPtp1, 1);
...
//  myfunc1->SetMaximum(1.5);
 ...
 TLatex latex2;
  ...
  latex.DrawLatex(0.57,0.32,"#bf{p + #bar{p}}");
  ...
  Ratio->GetYaxis()->SetRangeUser(0.2 ,1.3);
