...
void RT_ST_Kpm_2()
{
  ...
  TProfile *hprof1  = new TProfile("hprof1","ST",100,0,4,0,5);      // rope hadronization
  TProfile *hprof2  = new TProfile("hprof2","RT",100,0,4,0,5);      // rope hadronization
  ...
  TTree *rtree2 = (TTree*)input2->Get("tree2");

  TTree *rttree1 = (TTree*)input31->Get("tree1");
  TTree *sttree1 = (TTree*)input32->Get("tree1");

  TTree *pttree2 = (TTree*)input4->Get("tree2");


  int id, event, size, no, Neve;
  double n_density2, n_charged2, density_factor, Nch, Rt, St, pT_sum2, meanPtkpm2, pT_lead_lower_cut, pT_lead_upper_cut;
  double pT_lead, eta_2, y_2, phi_2;


  int entries = rtree5 ->GetEntries();
  int enteries_2 = rtree2 ->GetEntries();
  int enteries_4 = rtree4 ->GetEntries();

  cout << entries << endl;
  cout << enteries_2 << endl;
  cout << enteries_4 << endl;


  rtree4 ->SetBranchAddress("n_density2", &n_density2);  // Transverse momentum for charged particle
  rtree4 ->SetBranchAddress("pT_lead", &pT_lead);
  rtree4 ->SetBranchAddress("pT_sum2", &pT_sum2);
  rtree2 ->SetBranchAddress("eta_2", &eta_2);

  rttree1 ->SetBranchAddress("Rt", &Rt);
  sttree1 ->SetBranchAddress("St", &St);
  pttree2 ->SetBranchAddress("meanPtkpm2", &meanPtkpm2);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtkpm2, 1);
      hprof2->Fill(Rt, meanPtkpm2, 1);
 ...
 TLatex latex2;
  ...
  latex.DrawLatex(0.57,0.82,"#bf{K^{+} + K^{-}}");

  latex2.SetNDC(1);                                    // Set NDC
  latex2.SetTextSize(0.06);
  latex2.SetTextAlign(13);
  latex2.DrawLatex(0.57,0.72,"#bf{Toward Region}");
