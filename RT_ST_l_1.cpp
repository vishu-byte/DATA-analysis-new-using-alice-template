...
void RT_ST_l_1()
{
  ...
  double n_density1, n_charged1, density_factor, Nch, Rt, St, pT_sum1, meanPtl1, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  rtree4 ->SetBranchAddress("n_density1", &n_density1);  // Transverse momentum for charged particle
  ...
  pttree1 ->SetBranchAddress("meanPtl1", &meanPtl1);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtl1, 1);
      hprof2->Fill(Rt, meanPtl1, 1);
...
//  myfunc1->SetMaximum(1.5);
  ...
  legend->AddEntry(myfunc2, "Y = R_{T}", "l");
  legend->Draw();
 ...
 TLatex latex2;
  ...
  latex.DrawLatex(0.57,0.32,"#bf{#Lambda^{0} + #bar{#Lambda^{0}}}");
  ...
  Ratio->GetYaxis()->SetRangeUser(0.2 ,1.2);
