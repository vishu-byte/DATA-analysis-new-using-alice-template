...
// 1)Creating the canvas: (generic process)
  ...
  TFile *input4 = new TFile("../ROOT_RT_Analysis/meanPts.root",
                            "read"); // Reading the data.root file  (With_RH)
 // Reading the data.root file  (With_RH)
  ...
  double n_density3, n_charged3, density_factor, Nch, Rt, St, pT_sum3, meanPtkpm3, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  pttree3 ->SetBranchAddress("meanPtkpm3", &meanPtkpm3);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtkpm3, 1);
      hprof2->Fill(Rt, meanPtkpm3, 1);
