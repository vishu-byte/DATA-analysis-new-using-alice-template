...
void p_multi_ST() {
  ...
  int id, event, size, no, np1, np2, np3;
  ...
  mtree1->SetBranchAddress("np1", &np1);
  mtree2->SetBranchAddress("np2", &np2);
  mtree3->SetBranchAddress("np3", &np3);
  ...
  for (int i = 0; i < entries; i++) {
    ...
    if (pT_lead > pT_lead_lower_cut && pT_lead < pT_lead_upper_cut) {
      hprof1->Fill(St, np1, 1);
      hprof2->Fill(St, np2, 1);
      hprof3->Fill(St, np3, 1);
    }
  }
  ...
  latex.DrawLatex(0.70, 0.82, "#bf{p + #bar{p}}");
