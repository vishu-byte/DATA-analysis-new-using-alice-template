...
void Ks_multi_ST() {
  ...
  int id, event, size, no, nKs1, nKs2, nKs3;
  ...
  mtree1->SetBranchAddress("nKs1", &nKs1);
  mtree2->SetBranchAddress("nKs2", &nKs2);
  mtree3->SetBranchAddress("nKs3", &nKs3);
  ...
  for (int i = 0; i < entries; i++) {
    ...
    if (pT_lead > pT_lead_lower_cut && pT_lead < pT_lead_upper_cut) {
      hprof1->Fill(St, nKs1, 1);
      hprof2->Fill(St, nKs2, 1);
      hprof3->Fill(St, nKs3, 1);
    }
  }
  ...
  latex.DrawLatex(0.60, 0.82, "#bf{K_{s}^{0} + #bar{K_{s}^{0}}}");
