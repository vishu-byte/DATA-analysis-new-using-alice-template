...
void RT_ST_pi_3()
{
  ...
  double n_density3, n_charged3, density_factor, Nch, Rt, St, pT_sum3, meanPtpi3, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  pttree3 ->SetBranchAddress("meanPtpi3", &meanPtpi3);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtpi3, 1);
      hprof2->Fill(Rt, meanPtpi3, 1);
  ...
  myfunc1->SetMaximum(1.0);
  myfunc1->SetMinimum(0.6);
  myfunc1->SetTitle("");
