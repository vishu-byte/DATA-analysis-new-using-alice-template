...
void pi_multi_ST() {
  ...
  TFile *input = new TFile("../ROOT_RT_Analysis/ST.root",
  ...
  hprof2->SetMinimum(0);
  hprof2->SetMaximum(15.0);

  TTree *stree1 = (TTree *)input->Get("tree1"); // to get St
  TTree *mtree1 =
      (TTree *)input2->Get("tree1"); // to get npions from meanPts.root
  TTree *mtree2 = (TTree *)input2->Get("tree2");
  TTree *mtree3 = (TTree *)input2->Get("tree3");
  TTree *rtree4 = (TTree *)input3->Get("tree4");

  int id, event, size, no, npion1, npion2, npion3;
  double density_factor, St, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  stree1->SetBranchAddress("St", &St);
  mtree1->SetBranchAddress("npion1", &npion1);
  mtree2->SetBranchAddress("npion2", &npion2);
  mtree3->SetBranchAddress("npion3", &npion3);
  ...
  for (int i = 0; i < entries; i++) {
    ...
    if (pT_lead > pT_lead_lower_cut && pT_lead < pT_lead_upper_cut) {
      hprof1->Fill(St, npion1, 1);
      hprof2->Fill(St, npion2, 1);
      hprof3->Fill(St, npion3, 1);
    }
  }
  ...
  latex.DrawLatex(0.70, 0.82, "#bf{#pi^{+} + #pi^{-}}");
