...
void RT_ST_3()
{
  ...
  TProfile *hprof2  = new TProfile("hprof2","RT",25,0,5,0,5);      // rope hadronization
  ...
  hprof1->SetMinimum(0);
  hprof1->SetMaximum(3.0);

  hprof2->SetMinimum(0);
  hprof2->SetMaximum(3.0);
  ...
  double n_density3, n_charged3, density_factor, Nch, Rt, St, pT_sum3, meanPt3, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    meanPt3  = double(pT_sum3*density_factor/n_charged3);



    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPt3, 1);
      hprof2->Fill(Rt, meanPt3, 1);
  ...
  */
  ...
  legend->AddEntry(hprof1, "X = S_{T}", "l");
  legend->AddEntry(hprof2, "X = R_{T}", "l");
  legend->Draw();
  ...
  latex2.DrawLatex(0.57,0.82,"#bf{Away Region}");
