...
void ST_Ks_ratio_spectra3() {
  ...
  TFile *input2 = new TFile(" ../ROOT_RT_Analysis/ST.root",
  ...
  int entries =
      tree3->GetEntries(); // total number of enteries for variable pT_1
  ...
  for (int i = 0; i < entries; i++) {
    ...
    //--
    if (St >= 0.0 && St < 0.5) {
      if (abs(id_3) == 310) { // fill only for K0S particle
        hist->Fill(pT_3);
      }
    }
  ...
  legend->AddEntry(hist, "0. #leq S_{T} < 0.5 ", "lep");
  legend->AddEntry(hist2, "0.5 #leq S_{T} < 1.5", "lep");
  legend->AddEntry(hist3, "1.5 #leq S_{T} < 2.5", "lep");
  legend->AddEntry(hist4, "2.5 #leq S_{T} < 5.0", "lep");
  legend->Draw();
  ...
  latex.DrawLatex(0.67, 0.88, "#bf{K^{0}_{s} + #bar{K^{0}_{s}}}");
  ...
  //---
  latex2.DrawLatex(0.67, 0.72, "#bf{Away Region}");
  ...
  hs2->GetYaxis()->SetTitle("Ratio to S_{T} #geq 0");
