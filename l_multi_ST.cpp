...
void l_multi_ST() {
  ...
  int id, event, size, no, nl1, nl2, nl3;
  ...
  mtree1->SetBranchAddress("nl1", &nl1);
  mtree2->SetBranchAddress("nl2", &nl2);
  mtree3->SetBranchAddress("nl3", &nl3);
  ...
  for (int i = 0; i < entries; i++) {
    ...
    if (pT_lead > pT_lead_lower_cut && pT_lead < pT_lead_upper_cut) {
      hprof1->Fill(St, nl1, 1);
      hprof2->Fill(St, nl2, 1);
      hprof3->Fill(St, nl3, 1);
    }
  }
  ...
  latex.DrawLatex(0.60, 0.82, "#bf{#Lambda^{0} + #bar{#Lambda^{0}}}");
