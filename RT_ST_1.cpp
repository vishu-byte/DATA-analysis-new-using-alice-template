...
void RT_ST_1() {
  ...
  double n_density1, n_charged1, density_factor, Nch, Rt, St, pT_sum1, meanPt1,
  ...
  for (int i = 0; i < entries; i++) {
    ...
    meanPt1 = double(pT_sum1 * density_factor / n_charged1);

    if (pT_lead > pT_lead_lower_cut &&
        pT_lead < pT_lead_upper_cut) { // Analyze within pT lead cut
      hprof1->Fill(St, meanPt1, 1);
      hprof2->Fill(Rt, meanPt1, 1);
    }
  }
  ...
  */
  ...
  latex2.DrawLatex(0.57, 0.82, "#bf{Transverse Region}");
