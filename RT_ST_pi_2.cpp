...
void RT_ST_pi_2()
{
 ...
 /*0) Declare objects and call trees */
  ...
  double n_density2, n_charged2, density_factor, Nch, Rt, St, pT_sum2, meanPtpi2, pT_lead_lower_cut, pT_lead_upper_cut;
  ...
  pttree2 ->SetBranchAddress("meanPtpi2", &meanPtpi2);
  ...
  for (int i = 0; i < entries; i++)
  {
    ...
    if (pT_lead > pT_lead_lower_cut  && pT_lead < pT_lead_upper_cut ) {   // Analyze within pT lead cut
      hprof1->Fill(St, meanPtpi2, 1);
      hprof2->Fill(Rt, meanPtpi2, 1);
  ...
  myfunc1->SetMaximum(1.7);
  myfunc1->SetMinimum(0.8);
  myfunc1->SetTitle("");
 ...
 TLatex latex2;
  ...
  latex.DrawLatex(0.57,0.82,"#bf{#pi^{+} + #pi^{-}}");
