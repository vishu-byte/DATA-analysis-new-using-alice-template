...
void RT_Kpm_pi_spectra2()
{
  ...
  TFile *input2 = new TFile("RT.root", "read"); // Reading the data.root file
      ...
      if (Rt >=0.0 &&  Rt < 0.5)
        {
          ...
          if(abs(id_2) == 211 ) {         // fill only for K0S particle
            histpi->Fill(pT_2);}
...
cout << "Filling hists done" << endl;
  ...
  hs->Add(histkpmpi4);
    ...
    legend->AddEntry(histkpmpi1, "0. #leq R_{T} < 0.5 ", "l");
    legend->AddEntry(histkpmpi2, "0.5 #leq R_{T} < 1.5", "l");
    ...
    latex2.DrawLatex(0.15,0.82,"Toward Region");
